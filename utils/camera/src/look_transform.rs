use bevy::{ecs::prelude::*, math::prelude::*, prelude::ReflectDefault, reflect::Reflect, transform::components::Transform};
...
pub fn look_transform_system(mut cameras: Query<(&LookTransform, &mut Transform, Option<&mut Smoother>)>) {
    for (look_transform, mut scene_transform, smoother) in cameras.iter_mut() {
        if let Some(mut s) = smoother {
