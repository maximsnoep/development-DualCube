...
/// Multi-source Dijkstra with predecessor tracking, also skipping `exclude` vertices.
///
/// Returns (distances, predecessors) indexed by position in `region_verts`.
pub(super) fn restricted_dijkstra_with_pred(
    sources: &HashSet<VertID>,
    region_verts: &[VertID],
    vert_to_idx: &HashMap<VertID, usize>,
    exclude: &HashSet<VertID>,
    mesh: &Mesh<INPUT>,
    ...
    while let Some(Reverse((OrderedFloat(d), u_idx))) = heap.pop() {
        ...
        for nbr in mesh.neighbors(u) {
            ...
            // Don't route through excluded vertices (other boundary loops).
            if exclude.contains(&nbr) {
                continue;
            }
            let edge_len = (mesh.position(u) - mesh.position(nbr)).norm();
            let new_dist = d + edge_len;
...
/// Reconstructs the shortest path from a source (in `sources`) to `target`,
/// using the predecessor array from Dijkstra.
pub(super) fn reconstruct_path(
    ...
    let Some(&target_idx) = vert_to_idx.get(&target) else {
        return Vec::new();
    };
    let mut path = Vec::new();
    let mut current = target_idx;
    let max_steps = region_verts.len();
    for _ in 0..max_steps {
        path.push(region_verts[current]);
        if sources.contains(&region_verts[current]) {
            break;
        }
        match pred[current] {
            Some(p) => current = p,
