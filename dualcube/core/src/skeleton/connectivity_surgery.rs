...
use crate::skeleton::curve_skeleton::{patch_centroid, BoundaryLoop, SkeletonNode};
                ...
                for &v in neighbor_set {
                    ...
                    if u < v && !self.is_dead.contains(&v) {
                        if let Some(&v_node) = node_indices.get(&v) {
                            ...
                            let loop_edges: Vec<EdgeID> = compute_boundary_loop_between_patches(
                                &patch_u,
                                &patch_v,
                                original_mesh,
                            );
...
fn compute_boundary_loop_between_patches(
    patch_a: &[VertKey<INPUT>],
    patch_b: &[VertKey<INPUT>],
    mesh: &Mesh<INPUT>,
) -> Vec<EdgeID> {
    ...
    let b_inds: Vec<_> = sv.iter().copied().filter(|&v| !set_a.contains(&v)).collect();
    ...
    let mut expected_edges: HashSet<(VertKey<INPUT>, VertKey<INPUT>)> = HashSet::new();
    ...
    let traversed_edges: HashSet<(VertKey<INPUT>, VertKey<INPUT>)> = loop_edges
        .iter()
        .map(|&e| {
            let a = mesh.root(e);
            let b = mesh.toor(e);
