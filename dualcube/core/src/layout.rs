...
impl Layout {
                ...
                for &vert in &verts {
                    ...
                    for normal in normals {
                        for i in 0..face_labels.len() {
                            let angle =
                                to_vector(face_labels[i].0, face_labels[i].1).angle(&normal);
                            if angle < 1. {
    ...
    pub fn laplacian_corner_shoot(
        ...
        for polycube_neighbor in self.polycube_ref.structure.neighbors(polycube_vert) {
            let mesh_neighbor = self
                .vert_to_corner
                .get_by_left(&polycube_neighbor)
                .unwrap()
                .to_owned();
    ...
    fn compute_quality(&mut self) {
        ...
        let total_area: f64 = self
            ...
            .face_ids()
            .into_iter()
            .map(|f| self.granulated_mesh.size(f))
            .sum();
        self.alignment = Some(
            alignments
                .iter()
        ...
        let orthogonalities = polycube
            ...
            .flat_map(|&corner| {
                polycube
                    ...
                    .map(|face_id| {
                        polycube
                            .structure
                            .edges_in_face_with_vert(face_id, corner)
                            .unwrap()
                    })
                    .collect::<Vec<_>>()
            })
            .map(|[edge1, edge2]| {
                ...
                let vector1 = self.granulated_mesh.position(u_in_mesh)
                    - self.granulated_mesh.position(v_in_mesh);
                let vector2 = self.granulated_mesh.position(w_in_mesh)
                    - self.granulated_mesh.position(v_in_mesh);
        ...
        self.orthogonality =
            Some(orthogonalities.iter().cloned().sum::<f64>() / orthogonalities.len() as f64);
    }
}
